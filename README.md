...
_Do not close this terminal window, so you can see the Cassandras inner workings._
...
- When about 20 voters create votes concurrently response timeout from cassandra can be notices - when not properly handled occurs as "lost vote" in our case, see [docs/logs/cass_timeout.txt](docs/logs/cass_timeout.txt)
